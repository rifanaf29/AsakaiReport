...
function updateCapaAreaTitle(areaIndex) {
    ...
    const title = document.getElementById(`capa-area-title-${areaIndex}`);
    
    if (title) {
        let areaName = '';
        if (select && select.value && select.value !== '__custom__') {
            areaName = select.value;
        } else if (input && input.value) {
            areaName = input.value;
        }
        
        const displayName = areaName ? `CAPA Area ${areaName}` : `CAPA Area #${areaIndex + 1}`;
        const svg = title.querySelector('svg');
        if (svg) {
            title.innerHTML = svg.outerHTML + ' ' + displayName;
        } else {
...
function toggleCustomAreaInput(areaIndex) {
    ...
    if (select && input) {
        if (select.value === '__custom__') {
            ...
            updateCapaAreaTitle(areaIndex);
        }
    }
}

window.addCapaArea = function(areaName = null) {
    ...
    areaDiv.className = 'bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-lg mb-6 newly-added';
    areaDiv.id = `capa-area-${areaIndex}`;
    areaDiv.innerHTML = `
        <div class="px-5 py-4 bg-orange-50 dark:bg-orange-900/20 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <h4 id="capa-area-title-${areaIndex}" class="font-semibold text-gray-900 dark:text-gray-100 flex items-center">
            ...
            <div class="flex items-center space-x-2">
                <button type="button" onclick="toggleCapaAreaCollapse(${areaIndex})" 
                        class="text-gray-700 dark:text-gray-200 hover:text-gray-900 dark:hover:text-white hover:bg-white/60 dark:hover:bg-white/10 rounded-lg p-2 transition-colors"
                        title="Collapse/Expand">
                    <svg id="collapse-icon-${areaIndex}" class="w-5 h-5 collapse-icon collapsed" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </button>
                <button type="button" onclick="removeCapaArea(${areaIndex})" 
                        class="text-gray-700 dark:text-gray-200 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg p-2 transition-colors"
                        title="Remove Area">
        ...
        <div id="capa-content-${areaIndex}" class="capa-area-content collapsed">
            <div class="p-5">
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label onblur="updateCapaAreaTitle(${areaIndex})"
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               class="hidden mt-2 w-full px-4 py-3 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="capa_areas[${areaIndex}][area_description]" rows="2"
                              ...
                              class="w-full px-4 py-3 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200"></textarea>
                </div>
...
window.addProblem = function(areaIndex) {
    ...
    problemDiv.className = 'problem-card bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-md newly-added';
    problemDiv.id = `problem-${areaIndex}-${problemCount}`;
    problemDiv.innerHTML = `
        <div class="px-5 py-4 bg-red-50 dark:bg-red-900/20 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <button type="button" onclick="addCause(${areaIndex}, ${problemCount})" 
                            class="inline-flex items-center px-3 py-2 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 shadow-sm transition-colors">
...
window.addCause = function(areaIndex, problemIndex) {
    ...
    causeDiv.className = 'cause-card bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 border border-gray-200 dark:border-gray-700 shadow-sm';
    causeDiv.id = `cause-${areaIndex}-${problemIndex}-${causeCount}`;
    causeDiv.innerHTML = `
        <div class="flex items-center justify-between mb-3">
            <h6 class="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                <svg class="w-4 h-4 mr-2 text-gray-700 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
            <div class="flex items-center justify-between mb-3">
                ...
                <button type="button" onclick="addActionPlan(${areaIndex}, ${problemIndex}, ${causeCount})" 
                        class="inline-flex items-center px-3 py-1.5 bg-indigo-600 text-white text-xs font-semibold rounded-lg hover:bg-indigo-700 shadow-sm transition-colors">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                    </svg>
                    Add Action
...
window.addActionPlan = function(areaIndex, problemIndex, causeIndex) {
    ...
    actionDiv.className = 'action-card bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 border border-gray-200 dark:border-gray-700 shadow-sm';
    actionDiv.id = `action-${areaIndex}-${problemIndex}-${causeIndex}-${actionCount}`;
    actionDiv.innerHTML = `
        <div class="flex items-center justify-between mb-3">
            <span class="text-sm font-semibold text-gray-900 dark:text-gray-100 flex items-center">
                <svg class="w-4 h-4 mr-2 text-gray-700 dark:text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-3">
            <div>
                ...
                <textarea name="capa_areas[${areaIndex}][problems][${problemIndex}][causes][${causeIndex}][action_plans][${actionCount}][description]" 
                          ...
                          class="w-full px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200"></textarea>
            </div>
            ...
            <div class="grid grid-cols-3 gap-3">
                <div>
                    ...
                    <input type="text" 
                           ...
                           required
                           class="w-full px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200">
                </div>
                ...
                <div>
                    ...
                    <select name="capa_areas[${areaIndex}][problems][${problemIndex}][causes][${causeIndex}][action_plans][${actionCount}][status]"
                            required
                            class="w-full px-4 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent dark:bg-gray-700 dark:text-gray-200">
