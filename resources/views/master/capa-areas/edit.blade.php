    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-4 mb-4">
                ...
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Edit CAPA Area</h1>
            </div>
        </div>

        <!-- Form -->
        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-xl">
            <form action="{{ route('master.capa-areas.update', $capaArea) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <input 
                        ...
                        name="area_name" 
                        id="area_name" 
                        value="{{ old('area_name', $capaArea->area_name) }}"
                ...
                <div class="mb-6">
                    ...
                    >
                        ...
                        <option value="{{ $dept->id }}" {{ old('department_id', $capaArea->department_id) == $dept->id ? 'selected' : '' }}>
                            {{ $dept->name }} ({{ $dept->code }})
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-6">
                    ...
                    >{{ old('description', $capaArea->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
